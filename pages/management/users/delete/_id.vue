  ...
  <Loading :fetch-state="$fetchState">
    <form v-if="data" slot="content" class="editor" @submit.prevent="del">
      ...
      <h1 class="editor--title">
        Подтвердите удаление <span>{{ data.fio }}</span>
      </h1>
...
export default Vue.extend({
  ...
  async fetch() {
    const data = await this.$axios.$get(process.env.apiUrl + `/users/${this.$route.params.id}`).catch((err) => {
  ...
  methods: {
    async del() {
      this.$data.response = await this.$axios.$delete(process.env.apiUrl + `/users/${this.$route.params.id}`).catch((err) => err.response)
