<template>
  <Loading :fetch-state="$fetchState">
    <form v-if="data" slot="content" class="editor" @submit.prevent="del">
      ...
      <h1 class="editor--title">Подтвердите удаление <span>звонка</span></h1>
...
export default Vue.extend({
  ...
  methods: {
    async del() {
      this.$data.response = await this.$axios.$delete(process.env.apiUrl + `/bells/${this.$route.params.id}`).catch((err) => err.response)
