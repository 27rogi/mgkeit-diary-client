...
export default Vue.extend({
  ...
  async fetch() {
    const data = await this.$axios.$get(process.env.apiUrl + `/homeworks/${this.$route.params.id}`).catch((err) => {
  ...
  methods: {
    async del() {
      this.$data.response = await this.$axios.$delete(process.env.apiUrl + `/homeworks/${this.$route.params.id}`).catch((err) => err.response)
