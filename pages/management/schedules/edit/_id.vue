...
export default Vue.extend({
  ...
  async fetch() {
    await Promise.all([
      this.$axios.$get(process.env.apiUrl + `/schedules/${this.$route.params.id}`),
      ...
      .then((res) => {
        this.$data.schedule = res[0]
        delete this.$data.schedule.scheduleId
        this.$data.schedule.weekDate = this.$moment(res[0].weekDate).format('YYYY-MM-DD')
        this.$data.results.subjects = res[1].items
        this.$data.results.groups = res[2].items
        this.$data.results.bells = res[3].items
        this.$data.results.teachers = res[4].items
      })
  },
  beforeMount() {
  ...
  methods: {
    ...
    async add() {
      this.$data.response = await this.$axios
        .$patch(process.env.apiUrl + `/schedules/${this.$route.params.id}`, this.schedule)
